...
contract FundRaise is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _poolID, uint256 _amount) external payable {
        ...
        require(pool.userLimit >= rewards[_poolID].members.add(1),"Deposit :: pool depositor reached");
        require((pool.startBlock <= block.number) && (pool.endBlock >= block.number),"Deposit block exceed");
        require((pool.minDeposit <= _amount) && (pool.maxDeposit >= user.amount.add(_amount)) || (pool.minDeposit.add(userFee) <= msg.value) && (pool.maxDeposit >= user.amount.add(msg.value).sub(userFee)),"Invalid amounts to deposit");
    ...
    function claim(uint256 _poolID) external  nonReentrant {
        PoolInfo storage pool = poolInfo[_poolID];
        require(rewards[_poolID].depositTokens > 0,"Claim :: pool tokens already claimed");
        ...
        if(address(pool.depositToken) == address(0x0)){
            require(payable(wallet).send(ownerTokens),"transfer to owner failed");
            require(payable(msg.sender).send(tokens.sub(ownerTokens)),"transfer to pool creator failed");
        } else {
            pool.depositToken.transfer(wallet, ownerTokens);
